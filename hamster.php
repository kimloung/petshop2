...
<div>
    <h1 class="title-background">
        <span class="title">
            <?php
            $title = "Sản phẩm cho hamster";
            if(isset($_GET['menu']))
            {
                if($_GET['menu'] == 1)
                    $title = "Thức ăn cho hamster";
                if($_GET['menu'] == 2)
                    $title = "Vật dụng cho hamster";
                if($_GET['menu'] == 3)
                    $title = "Chuồng, lồng cho hamster";
            }
            echo $title;
            ?>
        </span>
     ...
     <div class="container">
         <div id="sp">
             <?php
                $sql = "SELECT sp.*, sdt.madv, sdt.matl, km.giakhuyenmai FROM sanpham as sp JOIN sp_dv_tl as sdt ON sp.masp = sdt.masp LEFT JOIN spkhuyenmai AS km ON sp.masp = km.masp WHERE xoa=0 AND madv='hamster'";
                if (isset($_GET['menu']))
                {
                    if($_GET['menu'] == 0)
                        $sql = $sql;
